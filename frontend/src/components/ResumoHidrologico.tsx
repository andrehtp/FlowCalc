...
export const ResumoHidrologico = ({ dados }: ResumoHidrologicoProps) => {
  ...
  const [dataMinima, dataMaxima] = useMemo(() => {
    ...
    return [ordenadas[0].dataInicial, ordenadas[ordenadas.length - 1].dataInicial];
  }, [dados]);
  ...
  return (
    <div className="bg-white p-4 rounded shadow">
      ...
      <h2 className="text-lg font-semibold mb-4">
        ...
        <span className="font-normal">{formatarDataBR(dataMinima)} até {formatarDataBR(dataMaxima)}</span>
      </h2>
...
<div className="grid grid-cols-2 md:grid-cols-10 gap-4 mb-6">
  ...
  <div className="bg-blue-50 rounded-md p-2 text-center shadow-sm border border-blue-100">
    ...
    <p className="text-base font-semibold">
      {(dados.reduce((acc, d) => acc + d.vazaoMedia, 0) / dados.length).toFixed(2)}
    </p>
  </div>
  <div className="bg-blue-50 rounded-md p-2 text-center shadow-sm border border-blue-100">
          ...
          <tbody>
            {dadosPagina.map((linha, index) => (
              <tr key={index} className="text-center">
                <td className="border px-2 py-1">{formatarDataBR(linha.dataInicial)}</td>
                <td className="border px-2 py-1">{linha.vazaoMedia ?? '-'}</td>
                <td className="border px-2 py-1">{linha.vazaoMaxima ?? '-'}</td>
                <td className="border px-2 py-1">{linha.vazaoMinima ?? '-'}</td>
                <td className="border px-2 py-1">{linha.vazaoMediaReal ?? '-'}</td>
                <td className="border px-2 py-1">{linha.vazaoMaximaReal ?? '-'}</td>
                <td className="border px-2 py-1">{linha.vazaoMinimaReal ?? '-'}</td>
                <td className="border px-2 py-1">{linha.nivelConsistencia ?? '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
